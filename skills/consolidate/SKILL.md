...
- **Eviction audit (the OUT valve — symmetric with the graduation queue above).** Graduation is one-way without this; left unchecked, the always-on `~/.claude/CLAUDE.md` (~6.2K tokens, loaded every session everywhere) slowly re-bloats. Same pass that promotes also evicts. Run two cheap checks; **surface candidates to Nick only when one fires** (silent otherwise — like the graduation queue):
  ...
  - **Trigger A — budget cap.** If CLAUDE.md exceeds **~7,500 tokens (~30KB)** (`wc -c ~/.claude/CLAUDE.md`), the audit MUST surface the **least-recently-reinforced** directives until the projected size is back under budget. A forcing function under real pressure. **Tie-break (matters on the first audit, when grandfathering has stamped every directive with the same `last_reinforced` date):** when `last_reinforced` ties, order by lowest reinforcement `count` first, then by largest byte-size (biggest bloat reduction per eviction). Without this, "least-recently-reinforced" is a 22-way tie on the first run and the selection is undefined.

...
- **Surface the graduation queue.** If `$SD/claude-md-candidates.md` exists and is non-empty, present its lines to Nick and ask which (if any) to graduate into `~/.claude/CLAUDE.md`. For each he approves, add a single compressed directive line to the appropriate section of `~/.claude/CLAUDE.md` with a pointer to the long-form `feedback_*.md` — never paste the body. Lines he declines stay in their `feedback_*.md` (no loss); a `(recurred: …)` annotation is a strong nudge to promote. This is the ONLY path by which a lesson reaches always-on, cascade-everywhere status — do not edit CLAUDE.md without his approval. If the file is absent, say nothing (no universal/meta lessons this session).

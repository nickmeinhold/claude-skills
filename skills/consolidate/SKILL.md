...
- **Eviction audit (the OUT valve — symmetric with the graduation queue above).** Graduation is one-way without this; left unchecked, the always-on `~/.claude/CLAUDE.md` (~6.2K tokens, loaded every session everywhere) slowly re-bloats. Same pass that promotes also evicts. Run two cheap checks; **surface candidates to Nick only when one fires** (silent otherwise — like the graduation queue):
  - **Seed + age the tracking.** Enumerate graduated directives by their `<!-- dir-id: xxxx -->` markers (`grep -oE '<!-- dir-id: [0-9a-f]{4} -->' ~/.claude/CLAUDE.md`). For each `dir-id`, look up its entry in `~/.claude/directive-reinforcement.json` (the sidecar step 1a stamps, keyed by the same `dir-id`). If a `dir-id` has NO entry, create one with `last_reinforced` = today — this **grandfathers** existing directives (first audit stamps everything today; a directive only ages toward staleness if N days pass with no NEW reinforcement). Never evict on first-seen. **FAIL SAFE for unmarked directives: a graduated-looking directive line (bold lead + `feedback_*.md` pointer) that carries NO `dir-id` marker is NEVER an eviction candidate** — it cannot be aged because it has no stable key, so treat it as freshly-grandfathered and surface it to Nick as "needs a dir-id backfill" instead. A missing marker must always degrade to keep, never to evict. (Orphan keys — sidecar entries whose `dir-id` no longer appears in CLAUDE.md — are dead weight, not eviction signals; note them for prune, do not act on them as staleness.)

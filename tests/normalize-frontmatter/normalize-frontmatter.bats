...
@test "scope: universal is preserved, never clobbered to repo" {
  ...
  run bash "$SCRIPT" "$f"
  [ "$status" -eq 0 ]
  [ "$(echo "$output" | meta_field scope)" = "universal" ]
}
...
@test "scope: meta is preserved" {
  ...
  run bash "$SCRIPT" "$f"
  [ "$status" -eq 0 ]
  [ "$(echo "$output" | meta_field scope)" = "meta" ]
}
...
@test "absent scope defaults to repo; an invalid scope also falls back to repo" {
  ...
  run bash "$SCRIPT" "$f"
  [ "$status" -eq 0 ]
  [ "$(echo "$output" | meta_field scope)" = "repo" ]
}
...
@test "existing curated type is preserved (type: user stays user)" {
  ...
  run bash "$SCRIPT" "$f"
  [ "$status" -eq 0 ]
  [ "$(echo "$output" | meta_field type)" = "user" ]
}
...
@test "an unfenced file (no frontmatter) triggers the mocked claude fallback" {
  ...
  [[ "$output" == *"Mocked Title"* ]]
  [[ "$output" == *"Mocked retrieval cue"* ]]
  [ "$(echo "$output" | meta_field type)" = "concept" ]
}
